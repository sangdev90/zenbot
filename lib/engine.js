      ...
      syncBalance(function (err) {
        ...
        getQuote(function (err, quote) {
          ...
          if (signal === 'buy') {
            ...
            if (!size) {
              if(so.max_asset === 0) {
                if(s.balance.currency >= so.max_asset) {
                  size = n(so.max_asset).divide(price).format('0.00000000')
                }
                else {
                  size = n(s.balance.currency).divide(price).format('0.00000000')
                }
              }
              else {
            ...
            if(so.max_asset && n(s.balance.asset).multiply(price) >= so.max_asset) {
              return cb(null, null)
            }
